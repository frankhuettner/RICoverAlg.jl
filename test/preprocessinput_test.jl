...
@test checkForTriviality([2. 1.],[.1; 0.9],1) == (ones(1), log(2.)*.1+log(1.)*.9, 0, "only one action possible")
@test checkForTriviality([2. ; 1.],[1],2) == ([1. ; 0.], log(2.0), 0, "only one state possible")
...
@test transform_utilities(U[:,[1,4]],lmd) == ( exp.((U[:,[1,4]].-.5)/lmd)   ,  .5   )
