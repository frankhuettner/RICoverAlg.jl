...
function checkForTriviality(Z,g,m)
    ...
    if m==1  ## since only one action possible
        return ones(m), (log.(Z)*g)[1], 0, "only one action possible"
    end
    if typeof(Z)==Vector{Float64}  ## since only one state possible
        best_solution = findmax(Z[:,1])
        p = zeros(m)
        p[best_solution[2]]=1
        return p, log(best_solution[1]), 0, "only one state possible"
    end
    return "not trivial"
end
...
function checkForBoundarySolution(Z,g)
    ...
    if (Z[1,:]./Z[2,:])'*g<1
        return [0, 1.0], log.(Z[2,:])'*g, 0,  "Boundary solution for two alternatives, zero iterations"
    elseif (Z[2,:]./Z[1,:])'*g<1
        return [1.0, 0], log.(Z[1,:])'*g, 0,  "Boundary solution for two alternatives, zero iterations"
    end
...
function preprocessinput(U, g, lmd)
    m = size(U)[1]
    solution = (ones(m)/m,0,0,"computation necessary")
